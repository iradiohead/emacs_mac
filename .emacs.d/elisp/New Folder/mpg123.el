...
(defvar mpg123-ogg123-id-coding-system
  (and (fboundp 'coding-system-p)
       (cond ((coding-system-p '*euc-cn*) '*euc-cn*)
	     ((coding-system-p 'euc-cn) 'euc-cn)
	     (t nil)))
...
(defun mpg123:play (&optional startframe)
  ...
  (if (or (not (looking-at "[0-9]"))
    ...
    (let ((continue (equal mpg123*cur-music-number (mpg123:get-music-number)))
      ...
      ;(message (concat "Current No is" (int-to-string mpg123*cur-music-number)))
      (skip-chars-forward "^ ")
      (skip-chars-forward " ")
	...
	(let ((time (buffer-substring
	 ...
	 'mpg123:initial-filter))
	  ...
	  (message "%s %s.."
		   ...
		   (mpg123:get-music-info mpg123*cur-music-number 'name)))
      (lrc-open (mpg123:get-music-info mpg123*cur-music-number 'name))
...
(defun mpg123-display-slider ()
  ...
  (let*((selw (selected-window))
	...
	w redrawp (i 300))
    (unwind-protect
	(if (window-live-p mpgw)
	    (setq mpgf (window-frame mpgw)))
